...
def kernel_Matern3_2_1D(T_V, l):
  TV2 = (T_V ** 2)

  def k(x1, x2):
    r = np.sqrt(3 * (diffs(x1, x2) / l)**2)
    return TV2 * (1 + r) * np.exp(- r)
...
def predictor(kernel_params, noise_var_fn, observed_data):
  observed_xs, observed_ys = observed_data
  k, mu = kernel_params
  K = k(observed_xs, observed_xs) + np.diag(noise_var_fn(observed_xs))
  L = np.linalg.cholesky(K)

  def predict(test_xs):
    k_test = k(test_xs, observed_xs)
    y_s = np.linalg.solve(L,
            np.linalg.solve(L.transpose(),
              (observed_ys - mu(observed_xs))))
    predicted_mean = mu(test_xs) + np.matmul(k_test, y_s)
